...
<script>
document.getElementById("qpapers-button").className+= " anotherClass1";
</script>
		<div class="col-sm-9"  id="right-bar">
		<div id="qpapers-list">
		<div class="form-style-10">
		<form method="post" enctype="multipart/form-data">
		    <h1>T1 Question Paper<span>Select Question Paper to upload </span></h1>
			<div class="button-section">
		    <input type="file" name="fileToUpload" id="fileToUpload"><br>
		    <label>Description</label>
		    <textarea name="description" rows="4" cols="70"></textarea><br>
		    <input type="submit" value="Upload" id="references_submit"name="uploadQuestionPaper"><br><br>
		    </div>
		</form>
		<div name="all-references">
			<table style="padding:10px;">
				<tr><th class="col-xs-3">Previous Question Papers</th><th class="col-xs-5">Description</th><th class="col-xs-1"></th></tr>
				<?php	
$sql="SELECT * FROM DOCUMENTS WHERE course_id='$course_id' and doc_type='t1_quespaper'";
$result = mysqli_query($conn,$sql);
if(mysqli_num_rows($result) > 0)
{
...
{
	echo 'No Question Papers uploaded';
}
				?>
			</table>
		</div>
		...
		$target_dir = "uploads/$course_id/$course_path/question_papers/";
		...
		$qry="INSERT INTO DOCUMENTS (faculty_id,course_id,doc_type,doc_name,doc_path,description,year,semester) VALUES ('$user','$course_id','t1_quespaper','$filename','$target_file','$description',$year,$sem)";
		...
		?>
		</div><!-- List of Qpaper Uploads --><!-- List of Qpaper Uploads -->
		</div>	<!--Right bar sm-9 -->
		</div>	<!-- row-->
	</div>	<!-- container-fluid -->
	</body>
