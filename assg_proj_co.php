...
<html lang="en">
<head>
        <title> Assignment/Project | CO</title>
...
<script>
document.getElementById("co-button").className+= " anotherClass";
</script>
<?php
$sql = "SELECT * from CO_QUES where course_id = '$course_id' and test=4";
$result=mysqli_query($conn,$sql);
if(mysqli_num_rows($result)>0)
{
echo "<script>
		...
		<div class="col-sm-9"  id="right-bar">
			<div id="co-list">
				<div class="form-style-10">
					<form method="post">
		 	   			<h1>Assignment/Project Course Outcome Attainment</h1>
						<input type = "number" name="numques" placeholder = "Number of Assignments">
				...
				<?php
					...
					{	
						echo "<div class='form-style-10'><form method='post' action='assg_proj_co2.php'>";
						echo "<table>";
						$k = $_POST[numques];
						$_SESSION[num] = $k;
						for($j=1;$j<=$k;$j++)
...
<?php
	if(isset($_POST['uploadStudent']))
		...
		$handle = fopen($file, "r");
		$c = 0;
		$sql = "select count(*) from CO_QUES where course_id='" . $course_id . "' and test=4";
		$result=mysqli_query($conn,$sql);
...
<?php
		$sql="CREATE VIEW CO_QUES_MAPPING AS (select c.course_id,c.co_id,c.ques,m.student_id,m.mark,c.test from CO_QUES as c join MARKS as m on m.ques=c.ques and c.test=4 and m.test=4 and c.course_id='$course_id' and m.course_id='$course_id')";
		mysqli_query($conn,$sql);
...
$sql = "UPDATE COURSE_CO SET ass=$score where co_id=$row[cot] and course_id = '$course_id'";
mysqli_query($conn,$sql);
}
